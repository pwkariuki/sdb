...
namespace {
    ...
    // register to find one that has no bits set; return 0, 1, 2 or 3 if found
    ...
    // Enable catchpoints for system calls
    ...
    if (is_attached_ and state_ == process_state::stopped) {
        ...
        if (reason.info == SIGTRAP) {
            if (reason.trap_reason == trap_type::software_break and
                ...
                if (id.index() == 1) {
                    watchpoints_.get_by_id(std::get<1>(id));
                }
...
void sdb::process::augment_stop_reason(stop_reason &reason) {
    ...
    if (reason.info == (SIGTRAP | 0x80)) { // Fill in syscall information
        ...
        if (expecting_syscall_exit_) {
            ...
            for (auto i = 0; i < 6; ++i) {
                sys_info.args[i] = regs.read_by_id_as<uint64_t>(arg_regs[i]);
            }
...
sdb::stop_reason sdb::process::maybe_resume_from_syscall(
    ...
    if (syscall_catch_policy_.get_mode() == syscall_catch_policy::some) {
        auto& to_catch = syscall_catch_policy_.get_to_catch();
        auto found = std::find(
            std::begin(to_catch), std::end(to_catch), reason.syscall_info->id);
...
// Check the debug status register (DR6) to see which breakpoint was hit
// Check corresponding register in the range DR0-DR7 to get address to locate break/watchpoint
std::variant<sdb::breakpoint_site::id_type, sdb::watchpoint::id_type>
    ...
    using ret = std::variant<breakpoint_site::id_type, watchpoint::id_type>;
...
sdb::watchpoint& sdb::process::create_watchpoint(
    virt_addr address, stoppoint_mode mode, std::size_t size) {
