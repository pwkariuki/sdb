...
namespace {
    // Retrieve execution state from `/proc/pid/stat`
    ...
    // from the output of the `readelf` tool
    std::int64_t get_section_load_bias(
     ...
     // Retrieve the file offset of entry point of elf file
    ...
    // Get load address by parsing the `/proc/<pid>/maps` file
    virt_addr get_load_address(pid_t pid, std::uint64_t offset) {
...
TEST_CASE("Hardware breakpoints evade memory checksums", "[breakpoint]") {
    bool close_on_exec = false;
    sdb::pipe channel(close_on_exec);
    auto proc = process::launch(
...
TEST_CASE("Watchpoint detects read", "[watchpoint]") {
    ...
    proc->resume();
    proc->wait_on_signal(); // hit watchpoint
    ...
    proc->resume();
    auto reason = proc->wait_on_signal(); // hit software breakpoint
